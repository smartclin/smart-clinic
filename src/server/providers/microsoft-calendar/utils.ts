...
export function toMicrosoftDate({ value, originalTimeZone }: ToMicrosoftDateOptions) {
	...
	if (value instanceof Temporal.Instant) {
		const dateTime = value.toZonedDateTimeISO('UTC').toPlainDateTime().toString()
	...
	return {
		...
		timeZone:
			originalTimeZone?.parsed === value.timeZoneId ? originalTimeZone?.raw : value.timeZoneId,
	}
}
...
function parseMicrosoftAttendeeStatus(
	...
	if (status === 'notResponded' || status === 'none' || status === 'organizer') {
		return 'unknown'
	}
...
export function parseMicrosoftAttendee(attendee: MicrosoftEventAttendee): Attendee {
	return {
