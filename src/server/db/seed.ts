...
async function main() {
	...
	await db.user.deleteMany() // IMPORTANT: Uncomment this if User is an internal model you manage
	...
	internalUsers = await db.user.createManyAndReturn({
		data: usersToCreate,
	})
	...
	services = await db.services.createManyAndReturn({ data: servicesData })
	...
	for (const data of patientsToCreateData) {
		patients.push(await db.patient.create({ data }))
	...
	for (const data of doctorsToCreateData) {
		doctors.push(await db.doctor.create({ data }))
		await db.user.update({
	...
	for (const data of staffToCreateData) {
		staff.push(await db.staff.create({ data }))
		await db.user.update({
			where: { id: data.user.connect?.id }, // Access the ID through the connect object
			data: { role: data.role },
		})
	...
	const workingDays = await db.workingDays.createMany({
	...
	appointments = await db.appointment.createManyAndReturn({
	...
	medicalRecords = await db.medicalRecords.createManyAndReturn({
	...
	const vitalSigns = await db.vitalSigns.createMany({
		data: vitalSignsData,
	})
	...
	const diagnosis = await db.diagnosis.createMany({ data: diagnosisData })
	...
	const payments = await db.payment.createManyAndReturn({
		data: paymentsData,
	})
	...
	const patientBills = await db.patientBills.createMany({
	...
	const labTests = await db.labTest.createManyAndReturn({
		data: labTestsData,
	...
	const prescriptions = await db.prescription.createMany({
	...
	for (const patient of patients) {
		if (faker.datatype.boolean({ probability: NUM_VACCINATIONS_PROBABILITY })) {
			const adminStaff = faker.helpers.arrayElement(staff.filter(s => s.role === Role.STAFF)) // Only nurses administer
			if (adminStaff) {
	...
	const vaccinations = await db.vaccination.createMany({
	...
	const auditLogs = await db.auditLog.createMany({ data: auditLogsData })
	...
	const ratings = await db.rating.createMany({ data: ratingsData })
	...
	const whoGrowthStandard = await db.wHOGrowthStandard.createMany({
