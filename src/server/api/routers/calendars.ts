...
import { calendarProcedure, createTRPCRouter, protectedProcedure } from '../trpc'
...
export const calendarsRouter = createTRPCRouter({
	list: calendarProcedure.query(async ({ ctx }) => {
		...
		const defaultAccount =
			ctx.accounts.find(account => account.id === ctx.user.defaultAccountId) ?? ctx.accounts.at(0) // returns `undefined` if empty

		const calendars = accounts.flatMap(account => account.calendars)

		const defaultCalendar =
			calendars.find(calendar => calendar.id === ctx.user.defaultCalendarId) ??
			calendars.find(
				calendar => calendar.accountId === defaultAccount.accountId && calendar.primary,
			)
	...
	setDefault: calendarProcedure
		...
		.mutation(async ({ ctx, input }) => {
			const account = await ctx.providers.find(provider => provider.account.id === input.accountId)
