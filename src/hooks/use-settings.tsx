...
export const useSettings = () => {
	...
	const providers = useMemo(() => {
		const settingValue = data?.find(
			setting => setting.key === Constants.settingKeys.providers,
		)?.value
	...
	const systemPrompt = useMemo(() => {
		const settingValue = data?.find(
			setting => setting.key === Constants.settingKeys.systemPrompt,
		)?.value
	...
	const systemPromptForRag = useMemo(() => {
		const settingValue = data?.find(
			setting => setting.key === Constants.settingKeys.systemPromptForRag,
		)?.value
	...
	const systemPromptForRagSlim = useMemo(() => {
		const settingValue = data?.find(
			setting => setting.key === Constants.settingKeys.systemPromptForRagSlim,
		)?.value
	...
	const systemPromptForChatTitle = useMemo(() => {
		const settingValue = data?.find(
			setting => setting.key === Constants.settingKeys.systemPromptForChatTitle,
		)?.value
