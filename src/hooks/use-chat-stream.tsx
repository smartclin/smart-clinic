...
export const useChatStream = () => {
	...
	const updateLastChatsItem = (type: 'finish' | 'update', content = '', isReasoning = false) => {
		setChats(prevArray => {
	...
	const handleStreamChunk = (
		...
		const content = chatCompletionResponse.choices[0]?.delta.content
		const chunkReasoningContent =
			chatCompletionResponse.choices[0]?.delta.reasoning_content ??
			chatCompletionResponse.choices[0]?.delta.reasoning
		...
		if (
			hasReasoningContent &&
			isEmpty(chunkReasoningContent) &&
			hasNonWhitespaceChars(chunkContent)
		) {
			chatMessageChunks.push('</think>')
