	...
	return (
		<DayPicker
			...
			className={cn(
				...
				String.raw`rtl:**:[.rdp-button\Next>svg]:rotate-180`,
...
function CalendarDayButton({
	...
	return (
		<Button
			// --- FIX APPLIED HERE ---
			asChild={false} // Explicitly tell the Button not to render as a child, but as a button itself
			className={cn(
			...
			// `itemRef` is not a standard prop for a Button or a native HTML button.
			// It might be a custom prop from an older version or a different library.
			// Removing it for now, as it's likely causing a type error and is not used.
			// If it's crucial, you'll need to define it in `ButtonProps`.
			// itemRef={"ref"}
			size="icon"
			variant="ghost"
			{...props}
